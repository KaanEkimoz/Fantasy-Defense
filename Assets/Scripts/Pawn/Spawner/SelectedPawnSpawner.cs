...
namespace Pawn
{
    public class SelectedPawnSpawner : MonoBehaviour
    {
        ...
        public void SpawnThePawn(int pawnIndex)
        {
            ...
            if (Physics.Raycast (_myRay, out _hit)) //if myRay hits something, store all the info you can find in the raycasthit varible.
            {
                if ( PlaygroundGrid.Board.GetValue(_hit.point) == 0 && Input.GetMouseButtonDown(0) && visual) {// what to do if i press the left mouse button
                    Destroy(visual);
                    Destroy(activeMark);
                    ResetSelectionMarks();
        ...
        private bool GoldControl(GameObject pawnToSpawn)
        {
            ...
            {
                GoldController.Instance.SubstractGold(pawnToSpawn.GetComponent<Pawn>().pawn.pawnCost);
                return true;
            }
            else
            {
